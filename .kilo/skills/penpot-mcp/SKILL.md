...
Before any setup steps, verify that MCP is connected to an actual Penpot file. `high_level_overview` and `penpot_api_info` may return API documentation even when no design file is active, so a successful response from those tools does **not** prove a file is open.

Use `execute_code` to query live file state:

```javascript
return {
  currentFile: penpot.currentFile
    ? { id: penpot.currentFile.id, name: penpot.currentFile.name }
    : null,
  currentPage: penpot.currentPage
    ? { id: penpot.currentPage.id, name: penpot.currentPage.name }
    : null,
}
```

If `currentFile` and `currentPage` are present, the connection is active and you can skip setup. If either is `null`, ask the user to connect Penpot MCP via **File → MCP Server → Connect**.
