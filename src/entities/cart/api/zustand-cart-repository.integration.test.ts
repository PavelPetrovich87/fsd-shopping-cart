...
describe('ZustandCartRepository Integration', () => {
  ...
  it('T009 - save->get round-trip returns equivalent cart state', () => {
    ...
    zustandCartRepository.saveCart(cartWithItem)
    const retrieved = zustandCartRepository.getCart()
  ...
  it('T009 - round-trip preserves item quantity and price', () => {
    const original = createCart()
    const result = addItem(original, {
      skuId: 'SKU-002',
  ...
  it('T010 - sequential saves produce non-stale observable results', () => {
    const cart1 = createCart()
    zustandCartRepository.saveCart(cart1)
    const read1 = zustandCartRepository.getCart()
    ...
    zustandCartRepository.saveCart(result2.cart)
    const read2 = zustandCartRepository.getCart()
  ...
  it('T010 - empty-cart save path remains valid', () => {
    const emptyCart = createCart()
    zustandCartRepository.saveCart(emptyCart)
    const retrieved = zustandCartRepository.getCart()
  ...
  it('T010 - repeated empty-cart saves remain stable', () => {
    const emptyCart = createCart()
    zustandCartRepository.saveCart(emptyCart)
    const first = zustandCartRepository.getCart()

    zustandCartRepository.saveCart(emptyCart)
    const second = zustandCartRepository.getCart()

    expect(second.id).toBe(first.id)
    expect(second.items.size).toBe(0)
  })

  it('T011 - save-to-update completes within latency threshold', () => {
  ...
  it('T009 - repository adapter returns cart via reactive store state', () => {
    const cart = createCart()
    const result = addItem(cart, {
      skuId: 'SKU-REACTIVE',
      name: 'Reactive Test',
    ...
    const fromRepo = zustandCartRepository.getCart()
